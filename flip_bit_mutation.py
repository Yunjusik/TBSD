...
class FlipBitMutation(Mutation):
    ...
    '''
    def __init__(self, pm, NodeTrust, ShardNum):
        if pm <= 0.0 or pm > 1.0:
        ...
        if do_mutation:
            for i, genome in enumerate(individual.chromsome):
                ...
                if type(individual) is BinaryIndividual:
                    ...
                    tmp = [e for e,j in enumerate(self.NodeTrust) if j == self.NodeTrust[i]]
                    tmp2 = []
                    for i in range(self.ShardNum):
                        tmp2.append(self.ShardNum-1)
                    tmp3 = []
                    for e in tmp:
                        if not (individual.chromseome[e] in tmp3):
                            tmp3.append(individual.chromsome[e])
                    for x in tmp3:
                        tmp2.remove(x)
                    if not tmp2:
